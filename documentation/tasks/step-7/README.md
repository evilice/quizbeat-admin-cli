...
Сборка через `GET /compositions/:id/full` в этот этап не входит —
...
заметки читаются своим `GET /compositions/:id/notes`.
...
|---|--------|------------|------|
| 1 | Стор: список, создание, правка, порядок, удаление | этап 0 (клиент JSON и пустой `204`), этап 4 (корневой стор) | [01-notes-store.md](./01-notes-store.md) |
| 2 | Блок списка на карточке `/compositions/:id` | 1; карточка этапа 4; блоки этапов 5–6 уже на экране | [02-notes-list.md](./02-notes-list.md) |
| 3 | Создание и правка: оба перевода `text` | 2 | [03-note-form.md](./03-note-form.md) |
| 4 | Смена порядка (`noteIds` — полный набор) | 2 | [04-notes-reorder.md](./04-notes-reorder.md) |
| 5 | Удаление заметки с подтверждением | 2 | [05-notes-delete.md](./05-notes-delete.md) |
| 6 | Приёмка этапа 7 целиком | 3, 4, 5 | [06-step7-acceptance.md](./06-step7-acceptance.md) |
...
не выбирали их порознь. Вопросов, которые блокируют старт, нет.
...
- **Блок живёт на карточке `/compositions/:id`.** Второго экрана и
  ...
  экран правки; этапы 5–6 добавили аудио и картинки; этот этап
  добавляет блок заметок **ниже** блока картинок. Стык первой загрузки
  строки композиции не заменяется на `GET /full` — это
  [этап 8](../../03-technical-specification.md#этап-8--сборка-карточки-композиции).
  До него список — только `GET /compositions/:id/notes`.
