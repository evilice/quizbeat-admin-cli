...
| Тела `POST`/`PATCH`: `tagIds` массивом; снятие набора — `tagIds: []` | Диалог создания — [задача 3](./03-create-composition.md). Экран карточки — [задача 4](./04-composition-card.md) |
...
| Метод `GET /compositions/:id/full` — единственный способ прочитать одну композицию; разбор `null` у аудио-полей и `404` без очистки сессии | Отображение ответа `full` на экране — [задача 4](./04-composition-card.md). Аудио- и клип-методы — [этап 5](../../03-technical-specification.md#этап-5--аудио-и-отрезки) |
...
- Отдельного `GET /compositions/:id` нет: одна композиция читается только
  через `GET .../full`
  ...
  Метод возвращает ответ целиком (`clips`, `images`, `notes`,
  `originalAudioUrl`, `originalAudioDurationSec`), в том числе когда на
  экране ещё нет блоков этих сущностей (этапы 5–7 их не добавили) — поля
  просто не используются, пока для них нет UI. При отсутствии трека
  сервер отдаёт `originalAudioUrl: null` и `originalAudioDurationSec: null`,
  а не опускает поля (`getFull` не вызывает `getAudioUrl()`, тот бросил бы
  `404`) — это нормальное состояние свежей композиции, метод не
  подставляет пустую строку и не дёргает `GET .../audio` сам.
  `findOrThrow` фильтрует `deletedAt IS NULL`: мягко удалённая композиция
  на `GET .../full` — `404`, как несуществующий id, а не пустой агрегат.
  `fileUrl` внутри `clips` по-прежнему отсутствует в JSON, пока статус не
  `DONE` — не `null`; тип ответа не выдумывает обязательное
  `fileUrl: string | null`.
...
- Стор MobX, один на приложение, поле корневого стора. Методы ходят
  ...
  - полная карточка: `GET /compositions/:id/full`, успех `200` →
    разобранный `CompositionFullResponseDto` (или эквивалентный тип
    клиента) без побочных вызовов.
...
- Тесты на моке `fetch`:
  ...
  - метод `full`: один вызов на путь с суффиксом `/full`; ответ с
    массивами `clips` / `images` / `notes` (в том числе пустыми)
    принимается как есть; ответ с `originalAudioUrl: null` и
    `originalAudioDurationSec: null` — успех, после вызова в сети нет
    `GET .../audio`; `404` пробрасывается без очистки refresh.

## Критерии готовности

- Тесты из объёма работы проходят через `npm run test`.
- `npm run lint` проходит.
- В модуле стора есть пути списка, `POST`, `PATCH`, `DELETE`, `full` и
  нет отдельного чтения `GET /compositions/:id` без суффикса. Нет
  вызова `GET .../audio` из метода полной карточки. Проверяется чтением
  модуля и тестом на `null` URL.
