...
Аудио, отрезки, картинки, заметки и `GET /compositions/:id/full` в этот
этап не входят — [этап 5](../../03-technical-specification.md#этап-5--аудио-и-отрезки),
...
|---|--------|------------|------|
| 1 | Стор композиций: список, создание, правка, удаление | этап 1 (клиент с access и разбор ошибки) | [01-compositions-store.md](./01-compositions-store.md) |
| 2 | Список и пункт меню: поиск, фильтры, пагинация | 1; стор тегов этапа 3 (фильтр) | [02-compositions-list.md](./02-compositions-list.md) |
| 3 | Создание композиции | 2 | [03-create-composition.md](./03-create-composition.md) |
| 4 | Карточка: правка названия, автора, статуса и тегов | 2 | [04-composition-card.md](./04-composition-card.md) |
| 5 | Мягкое удаление с подтверждением | 2 | [05-delete-composition.md](./05-delete-composition.md) |
| 6 | Приёмка этапа 4 целиком | 3, 4, 5 | [06-step4-acceptance.md](./06-step4-acceptance.md) |
...
ТЗ задаёт набор вызовов и DoD и не фиксирует маршрут карточки, способ
её первой загрузки до появления `GET /full`, где живёт создание и как
собирается фильтр тегов. Ниже — решения, чтобы задачи не выбирали их
порознь. Вопросов, которые блокируют старт, нет. Отдельного
...
- **Список — `/compositions`, карточка — `/compositions/:id`.** Это
  ...
  добавят блоки, а [этап 8](../../03-technical-specification.md#этап-8--сборка-карточки-композиции)
  заменит способ первой загрузки на `GET /compositions/:id/full`.
  Второго экрана редактирования не заводить. Маршрут карточки заводит
  этот этап, даже без `GET /full`.
- **Первая загрузка карточки до этапа 8 — без `GET /full` и без
  выдуманного `GET /compositions/:id`.** Отдельного чтения одной
  композиции на сервере нет
  ([`CompositionsController`](../../../../quizbeat-srv/src/compositions/compositions.controller.ts)
  отдаёт список, мутации и `full`). Честное решение того же стыка:
  открытие из списка передаёт уже загруженную строку в состоянии
  навигации и копирует текущие query списка (`search`, `status`,
  `tagIds`, `page`, `limit`) в search карточки. Прямой заход и
  перезагрузка: экран заново читает `GET /compositions` с этими query и
  ищет строку с id из пути. Если строки нет (другая страница фильтра,
  уже удалена, чужой id) — состояние «не найдена» со ссылкой на список,
  не бесконечный скелет и не вызов `GET /full`. Это временный стык;
  [задача 4](./04-composition-card.md) пишет его явно, чтобы этап 8
  знал, что заменяет.
